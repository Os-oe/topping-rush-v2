...
class RedisStore {
  ...
  async rank(name) {
    // ZRANK kennt keine REV-Option (die gibt es nur bei ZRANGE) — die Option
    // wurde ignoriert und lieferte den AUFSTEIGENDEN Rang (niedrigster Score
    // = „Platz 1", live entdeckt 18.07.). Richtig ist ZREVRANK.
    const r = await this.redis.zrevrank('lb', name);
